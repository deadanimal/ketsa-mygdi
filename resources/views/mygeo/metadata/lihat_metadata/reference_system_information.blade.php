<div class="card card-primary div_c13" id="div_c13">
    ...
    <div id="collapse13" class="panel-collapse collapse in show" data-parent="#div_c13">
        <div class="card-body">
            <div class="row mb-2">
				<?php
				if(isset($refSysSelected) && !empty($refSysSelected)){
					?>
					<div class="col-xl-3">
						...
						foreach($template->template[strtolower($catSelected)]['accordion13'] as $key=>$val){
							if($val['status'] == "customInput"){
								...
								<div class="row mb-2 sortIt">
									...
									<div class="col-8">
										{{ (isset($metadataxml->customInputs->accordion13->$key) ? $metadataxml->customInputs->accordion13->$key:"") }}
									</div>
								</div>
								<?php
							}
						...
						if(isset($refSysSelected) && !empty($refSysSelected)){
							foreach($template->template[strtolower($catSelected)]['accordion13'] as $key=>$val){
								...
								if($key == "refsys_projection"){
									...
									$var = isset($refSysSelected['projection']) ? $refSysSelected['projection']:'';
									?>
								...
								if($key == "refsys_semiMajorAxis"){
									...
									$var = isset($refSysSelected['semi_major_axis']) ? $refSysSelected['semi_major_axis']:'';
									?>
									<label class="form-check-label" style="margin-left:20px;" <?php if($val['status'] == "inactive"){ ?>style="display:none;"<?php } ?>>
										<b>Semi Major Axis:</b>&nbsp;&nbsp;&nbsp;
								...
								if($key == "refsys_ellipsoid"){
									...
									$var = isset($refSysSelected['ellipsoid']) ? $refSysSelected['ellipsoid']:'';
									?>
									<label class="form-check-label" style="margin-left:20px;" <?php if($val['status'] == "inactive"){ ?>style="display:none;"<?php } ?>>
										<b>Ellipsoid:</b>&nbsp;&nbsp;&nbsp;
								...
								if($key == "refsys_axis_units"){
									...
									$var = isset($refSysSelected['axis_units']) ? $refSysSelected['axis_units']:'';
									?>
									<label class="form-check-label" style="margin-left:20px;" <?php if($val['status'] == "inactive"){ ?>style="display:none;"<?php } ?>>
										<b>Axis Units:</b>&nbsp;&nbsp;&nbsp;
								...
								if($key == "refsys_datum"){
									...
									$var = isset($refSysSelected['datum']) ?  $refSysSelected['datum']:'';
									?>
									<label class="form-check-label" style="margin-left:20px;" <?php if($val['status'] == "inactive"){ ?>style="display:none;"<?php } ?>>
										<b>Datum:</b>&nbsp;&nbsp;&nbsp;
								...
								if($key == "refsys_denomFlatRatio"){
									...
									$var = isset($refSysSelected['denominator_flattening_ratio']) ? $refSysSelected['denominator_flattening_ratio']:'';
									?>
									<label class="form-check-label" style="margin-left:20px;" <?php if($val['status'] == "inactive"){ ?>style="display:none;"<?php } ?>>
